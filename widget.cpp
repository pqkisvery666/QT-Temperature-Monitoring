...
void Widget::findall()
{
    model->setQuery(QString("select * FROM temperature"));
    //列名
...
void Widget::customsplot_init()
{
    ...
    if (key-lastPointKey > 0.002) //大于2ms添加一个数据
    {
        ...
        QSqlQuery query;
        QString str = QString("INSERT INTO temperature (date,tempe) values('%1','%2')").arg(date).arg(tempe);//删除表中符合id的所有数据
...
{
    QString date = ui->find_date->text();
    model->setQuery(QString("select * FROM temperature Where date = '%1'").arg(date));
    ui->tableView->setModel(model);//数据放置进去
}
...
{
    QString date = ui->otherdate->text();
    QString tempe = ui->tempe->text();
    QSqlQuery query;
    QString str = QString("INSERT INTO temperature (date,tempe) values('%1','%2')").arg(date).arg(tempe);//删除表中符合id的所有数据
    if (query.exec(str)==false)
    {
...
void Widget::on_del_clicked()
{
    QString date = ui->otherdate->text();
    QSqlQuery query;
    QString str = QString("DELETE FROM temperature WHERE date = '%1'").arg(date);//删除表中符合name的所有数据
    if (query.exec(str)==false)
    {
...
void Widget::on_update_clicked()
{
    ...
    QString tempe = ui->tempe->text();
    QSqlQuery query;
    QString str = QString("UPDATE temperature SET tempe='%1' WHERE date = '%2'").arg(tempe).arg(date);//删除表中符合id的所有数据
    if (query.exec(str)==false)
    {
